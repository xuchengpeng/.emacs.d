...
Meant for `+modeline-buffer-path-function'."
  (pcase-let
      ((`(,root-parent ,root ,dir, file)
        (shrink-path-file-mixed (dotemacs-project-root)
  ...
  (pcase status
    (`finished
     (let* ((error-counts (flycheck-count-errors flycheck-current-errors))
            ...
            (no-info (cdr (assq 'info error-counts))))
       (concat (propertize "•%s" (or no-errors 0) 'face 'dotemacs-modeline-urgent)
               " "
               (propertize "•%s" (or no-warnings 0) 'face 'dotemacs-modeline-warning)
               " "
               (propertize "•%s" (or no-info 0) 'face 'dotemacs-modeline-info))))
    (`interrupted "-")
